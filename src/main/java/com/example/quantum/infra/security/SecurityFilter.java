...
public class SecurityFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if(login != null){
            UserEntity entity = userRepository.findByUserId(UUID.fromString(login)).orElseThrow(() -> new RuntimeException("Usuário não encontrado!"));

            final var authorities = Collections.singletonList(new SimpleGrantedAuthority("ROLE_USER"));
            final var authentication = new UsernamePasswordAuthenticationToken(entity, null, authorities);
